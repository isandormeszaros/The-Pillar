...
//1.	GET		/brands			- Az összes elérhető óramárka lekérdezése
async function selectBrands(){
    return new Promise((resolve, reject) => {
        pool.query('select * from watches.base', (error, elements) => {
            if (error) {
                return reject(error);
            }
...
async function selectUsers(email,password){
    return new Promise((resolve,reject)=>{
        pool.query('select * from user where email=? and password=?',[email,password],(error, elements)=>{
            if(error){
                return(reject(error))
            }
            if(elements==""){
