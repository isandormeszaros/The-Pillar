...
/*!50001 VIEW `new_view2` AS select `base`.`brand_id` AS `brand_id`,`base`.`Model` AS `Model`,`brand`.`brand` AS `brand` from (`base` join `brand` on((`base`.`brand_id` = `brand`.`brand_id`))) */;
